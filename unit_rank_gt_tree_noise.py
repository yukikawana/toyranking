...
"""
ran = 100001.
neg = create_neg(input_shape,noise=True)[None,:,:,:]/ran
pos = create_pos(input_shape,noise=True)[None,:,:,:]/ran
for i in range(int(ran-1)):
    
    negtm = create_neg(input_shape,noise=True)[None,:,:,:]/ran
    postm = create_pos(input_shape,noise=True)[None,:,:,:]/ran
    pos+=postm
    neg+=negtm

pickle.dump([neg, pos], open('negpos.pkl', 'wb'))
"""
neg, pos = pickle.load(open('negpos.pkl','rb'))
...
with tf.Session() as sess:    
    ...
    #saver.restore(sess,'ckpts_manual_noise_gauss/1.ckpt')
    ...
    for name in posref:
        print name
        shape = posref[name].shape
        for i in range(shape[3]):
            print posref[name][0,:,:,i]
    posres = sess.run([net], feed_dict=fdpos)[0][0,0,0,:]
    negres = sess.run([net], feed_dict=fdneg)[0][0,0,0,:]
    print posres, negres
    negchs = OrderedDict()
    poschs = OrderedDict()
    posneg = OrderedDict()
