...
def sc(inputs, input_shape=(8,8), batch_size=1, modify=[]):
    ...
    name = 'conv1'
    net = slim.conv2d(inputs, 3, [3, 3],
    padding=padding,
    weights_initializer=initializer,
    weights_regularizer=regularizer,
    scope=name)
    c1 = tf.get_variable('scale1', shape=(3), initializer=initializer, regularizer=regularizer)
    c1 = c1 + 10
    ...
    net = slim.conv2d(net, 2, [3, 3],
        ...
        weights_regularizer=regularizer,
        scope=name)
    c1 = tf.get_variable('scale2', shape=(2), initializer=initializer, regularizer=regularizer)
    c1 = c1 + 10
    net = tf.multiply(net, c1)
    activations[name] = net
    if name in modify:
    ...
    net = slim.conv2d(net, 2, [3, 3],
        ...
        weights_regularizer=regularizer,
        scope=name)
    c1 = tf.get_variable('scale3', shape=(2), initializer=initializer, regularizer=regularizer)
    c1 = c1 + 10
    net = tf.multiply(net, c1)
    activations[name] = net
    if name in modify:
    ...
    net = slim.conv2d(net, 2, [3, 3],
        ...
        activation_fn=None,
        scope='conv4')
    c1 = tf.get_variable('scale4', shape=(2), initializer=initializer, regularizer=regularizer)
    c1 = c1 + 10
    net = tf.multiply(net, c1)
